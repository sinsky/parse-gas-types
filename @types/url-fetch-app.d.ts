...
@return The HTTP response data.*/ fetch(
    url: string,
    params: Object,
...
@param requests An array of either URLs or JavaScript objects specifying requests as defined
    below.
@return An array of HTTP response data from each input request.*/ fetchAll(
    requests: Object[],
  ): UrlFetchApp.HTTPResponse[];
...
@return A map of Field Name to Value. The map has at least the following keys: `url`,
    `method`, `contentType`, `payload`, and `headers`.*/ getRequest(
    url: string,
  ): Object;
...
@param charset a string representing the charset to be used for encoding the HTTP response
    content
@return the content of the HTTP response, encoded using the given charset*/ getContentText(
      charset: string,
    ): string;
...
```
@return HTTP response code (e.g. 200 for OK)*/ getResponseCode(): Integer;
  }
}
